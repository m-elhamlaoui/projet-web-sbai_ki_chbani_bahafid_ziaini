...
def index(request):
    ...
    produits = Produit.objects.annotate(
        totalNote=Avg("note__note"), totalU=Count("note__id_utilisateur")
    ).filter(qte_en_stock__gt=0)

    # Derniers produits (Wrapper 1, 2, 3)
    derniers_produits = Produit.objects.annotate(totalNote=Avg("note__note")).order_by(
        "-date_ajout"
    )[
        :12
    ]  # Récupère les 12 derniers produits
    ...
    # Meilleures ventes
    meilleures_ventes = (
        LigneCommande.objects.values("id_produit")
        .annotate(total=Sum("qte"))
        .order_by("-total")[:8]
    )
    produits_meilleures_ventes = Produit.objects.filter(
        id_produit__in=[mv["id_produit"] for mv in meilleures_ventes]
    )
...
def quickview(request, id_produit):
    ...
    notes = Note.objects.filter(id_produit=produit).aggregate(
        totalNote=Avg("note"), totalU=Count("id_utilisateur")
    )

    # Récupérer les avis associés au produit
    avis_produits = AvisProduit.objects.filter(id_produit=produit).select_related(
        "id_utilisateur"
    )
...
def product_detail(request, id_produit):
    ...
    # Récupérer les avis associés au produit
    avis_produits = AvisProduit.objects.filter(id_produit=produit).select_related(
        "id_utilisateur", "id_note"
    )

    # Calculer le total des commentaires
    total_commentaires = AvisProduit.objects.filter(id_produit=produit).aggregate(
        total=Count("id_avis_produit")
    )

    # Calculer la note moyenne du produit
    note_moyenne = Note.objects.filter(id_produit=produit).aggregate(
        moyenne=Avg("note")
    )
    ...
    if request.method == "POST":
        ...
        if avis_produit and id_utilisateur and id_produit and note:
            ...
            nouvelle_note = Note.objects.create(
                id_utilisateur=utilisateur, id_produit=produit, note=note
            )
            ...
            return JsonResponse(
                {
                    "status": "error",
                    "message": "Veuillez vous connecter pour pouvoir ajouter un avis sur un produit.",
                }
            )
    else:
        return JsonResponse(
            {"status": "errorFatal", "message": "Méthode non autorisée."}
        )
...
def recommander_produit(request):
    if request.method == "POST":
        ...
        if not user_id:
            ...
            return JsonResponse(
                {"status": "error", "message": "Utilisateur non connecté."}, status=401
            )
        ...
        if product_id and product_id != "-2":  # Vérifier que l'ID du produit est valide
            ...
            recommandation_exists = Recommandation.objects.filter(
                id_produit=product_id, id_utilisateur=user_id
            ).exists()
            ...
            if not recommandation_exists:
                ...
                Recommandation.objects.create(
                    id_produit=produit, id_utilisateur=utilisateur
                )

                return JsonResponse(
                    {"status": "success", "message": "Produit recommandé avec succès !"}
                )
            else:
                return JsonResponse(
                    {"status": "error", "message": "Produit deja recommande !"}
                )
        else:
            return JsonResponse(
                {"status": "error", "message": "ID de produit invalide."}, status=400
            )
    else:
        return JsonResponse(
...
def ajouter_avis_service(request):
    if request.method == "POST":
        ...
        if not avis:
            ...
            return JsonResponse(
                {"status": "error", "message": "Veuillez remplir le champ avis."}
            )
        ...
        AvisService.objects.create(id_utilisateur=utilisateur, avis_service=avis)

        return JsonResponse(
            {"status": "success", "message": "Avis enregistré avec succès !"}
        )
    else:
...
def categorie(request, id_categorie):
    ...
    derniers_produits = Produit.objects.order_by("-date_ajout")[
        :5
    ]  # Limite à 5 produits
    ...
    if request.method == "POST":
        ...
        if photo:
            ...
            photo_path = default_storage.save(
                f"users_epicerie/static/image/photos_utilisateurs/{pseudo}_{photo.name}",
        ...
        if response != "-1":
            if 'shopping_cart' in request.session:
                ...
                for item in shopping_cart:
                    ...
                    # Replace comma with period and convert to Decimal
                    try:
                        product_price_str = item['product_price'].replace(',', '.')  # Replace comma with period
                        product_price = Decimal(product_price_str)  # Convert to Decimal
                    except InvalidOperation:
                        return JsonResponse({'status': 'error', 'message': f"Prix invalide pour le produit {item['product_name']}."})
                ...
                if verification:
                    ...
                    for item in shopping_cart:
                        ...
                        Facture.objects.create(
                            ...
                            id_produit_id=product_id,
                            qte=product_quantity,
                            prix_produit=product_quantity * product_price,
    ...
    if request.method == "POST":
        ...
        if action == "add":
            ...
            product_quantity = int(
                request.POST.get("product_quantity", 1)
            ...
            shopping_cart = [
                item for item in shopping_cart if item["product_id"] != product_id
            ]
...
def fetch_cart(request):
    ...
    if 'shopping_cart' in request.session and request.session['shopping_cart']:
        ...
        for item in shopping_cart:
            ...
            product_price_str = item.get('product_price', '0').replace(',', '.')  # Replace comma with period
...
def passer_commande(request):
    if request.method == 'POST':
        ...
        for item in shopping_cart:
            ...
            # Replace comma with period and convert to Decimal
            try:
                prix_produit_str = item['product_price'].replace(',', '.')  # Replace comma with period
                prix_produit = Decimal(prix_produit_str)  # Convert to Decimal
            except InvalidOperation:
                return JsonResponse({'status': 'error', 'message': f"Prix invalide pour le produit {item['product_name']}."})
...
def mesCommandes(request):
    ...
    commandes = Commande.objects.filter(id_utilisateur=utilisateur).select_related(
        "id_utilisateur"
    )
...
def facture(request, id_commande):
    ...
    produits = Facture.objects.filter(id_commande=id_commande).select_related(
        "id_produit"
    )

    # Calculer le montant total de la facture
    total = Facture.objects.filter(id_commande=id_commande).aggregate(
...
def affiche_conversation_action(request):
    ...
    if messages.exists():
        ...
        # Aucun message
        html_response = (
            '<p style="color:white;font-size:1.6rem">Aucun message envoyé</p>'
        )
...
def enregistrer_message(request):
    ...
    if not id_recepteur or not id_envoyeur or not message:
        return JsonResponse(
            {"status": "error", "message": "Tous les champs sont obligatoires"},
            status=400,
        )
    ...
    try:
        ...
        nouveau_message = Messages.objects.create(
            id_envoyeur=id_envoyeur, id_recepteur=id_recepteur, message=message
        )

        # Retourner une réponse JSON en cas de succès
        return JsonResponse(
            {"status": "success", "message": "Message enregistré avec succès"}
        )
...
def utilisateurs_en_ligne(request):
    if request.method == "POST":
        ...
        if utilisateurs_avec_messages:
            ...
            for utilisateur in utilisateurs_avec_messages:
                ...
                dernier_message = (
                    Messages.objects.filter(
                        id_envoyeur=utilisateur.id_utilisateur, id_recepteur=0
                    )
                    .order_by("-date_envoi")
                    .first()
                )
            ...
            return JsonResponse(
                '<div class="alert danger-alert" style="color:white;font-size:1.5rem;"> Aucun Message <div>',
                safe=False,
            )
    ...
    if request.method == "POST":
        ...
        if valeur_recherche:
            ...
            if utilisateurs:
                ...
                return JsonResponse(
                    '<div class="alert danger-alert"> Aucun utilisateur trouvé <div>',
                    safe=False,
                )
        else:
...
def message_action_admin(request):
    if request.method == "POST":
        ...
        if not id_recepteur or not id_envoyeur or not message:
            return JsonResponse(
                {"error": "Tous les champs sont obligatoires."}, status=400
            )
        ...
        try:
            ...
            return JsonResponse(
                {"error": "Les ID doivent être des nombres valides."}, status=400
            )
        ...
        return JsonResponse(
            {
                ...
                "message_id": nouveau_message.id_message,
            }
        )
    else:
