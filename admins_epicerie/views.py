...
from gestion_epicerie.models import Admin, Utilisateur
...
from admins_epicerie.models import Categorie, Produit
...
def dashboard(request):
    ...
    chiffre_affaires = (
        LigneCommande.objects.annotate(
            total_line=F("qte") * F("id_produit__prix_promotion")
        ).aggregate(mtc=Sum("total_line"))["mtc"]
        or Decimal('0.00')
    )
    chiffre_affaires = Decimal(chiffre_affaires)
    ...
    profits_categorie = (
        Commande.objects.annotate(montant=F("montant_commande"))
        .values("lignecommande__id_produit__id_categorie__nom_categorie")
        .annotate(montant_total=Sum("montant_commande"))
    )
    # Convert montant_total to Decimal for each category
    for category in profits_categorie:
        category['montant_total'] = int(category['montant_total'] or '0.00')

    # 4. Total des commandes
    total_commandes = (
        Commande.objects.aggregate(totalCom=Count("id_commande"))["totalCom"] or 0
    )

    # 5. Total des produits vendus
    total_produits_vendus = (
        LigneCommande.objects.aggregate(vendu=Count("id_produit"))["vendu"] or 0
    )
    ...
    prix_total_stock = (
        ...
        ).aggregate(total=Sum("total_line"))["total"]
        or 0
    )

    # 7. Meilleurs clients
    meilleurs_clients = (
        Commande.objects.values(
            "id_utilisateur__photo",
            "id_utilisateur__nom",
    ...
    # Convert montant_total to Decimal for each client
    for client in meilleurs_clients:
        client['montant_total'] = Decimal(client['montant_total'] or '0.00')
    ...
    meilleures_notes = (
        Note.objects.values(
            ...
            "id_produit__prix_promotion",
            "id_produit__image_produit",
        )
        .annotate(noteP=Count("id_produit"))
        .order_by("-noteP")[:7]
    )
    ...
    plus_vendus = (
        LigneCommande.objects.values("id_produit__nom_produit")
        .annotate(nbVendu=Count("id_produit"))
        .order_by("-nbVendu")[:5]
    )
...
def ajout_solde_admin(request):
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse(
                {"erreur": "Pseudo inexistant, vérifiez bien le pseudo"}
            )
...
def produits_inpt(request):
    ...
    produits = produits.annotate(qte_vendue=Sum("lignecommande__qte"))
...
def commandes(request):
    ...
    commandes = (
        Commande.objects.filter(commande_pret="NON")
        .annotate(
            heure=F("date_commande__hour"),  # Extraire l'heure de la commande
            minute=F("date_commande__minute"),
        )
...
def produits(request):
    ...
    meilleurs_ventes = Produit.objects.annotate(
        total_vendu=Sum("lignecommande__qte")
    ).order_by("-total_vendu")[
        :5
    ]  # Trier par quantité vendue (descendant) et limiter à 5
...
def affiche_produits_admin(request):
    if request.method == "POST" and "id" in request.POST:
        ...
        html = render_to_string("produit_admin_partial.html",
                                {"produits": produits})
...
def supprimer_produit(_, id_produit):
    """
    Deletes a product based on its ID and redirects to the products page.

    Args:
        _: Unused parameter (request object, required by Django views).
        id_produit (int): The ID of the product to be deleted.

    Returns:
        HttpResponseRedirect: Redirects to the "produits" page after deletion.
    """
...
def modifier_produit(request, id_produit):
    """
    Handles the modification of a specific product by ID.

    Retrieves the product and its associated categories. If the request method
    is POST, validates and saves the form data to update the product.
    Otherwise, displays a pre-filled form for the product.

    Args:
        request (HttpRequest): The HTTP request object (GET or POST).
        id_produit (int): The ID of the product to be modified.

    Returns:
        HttpResponse: Renders the 'modifierProduit.html' template with the
                      context data, or redirects to the 'produits' page after
                      successful modification.
    """
    # Récupérer le produit spécifique ou renvoyer une erreur 404 s'il n'existe
    produit = get_object_or_404(Produit, id_produit=id_produit)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            # Rediriger vers la page des produits après la modification
            return redirect("produits")
    else:
...
def modifier_produit_ajax(request):
    """
    Handles the modification of a product via an AJAX request.

    Validates the input data and updates the product's details in the database.
    If an image is provided, it checks the file type and saves it. Returns a
    JSON response indicating success or failure.

    Args:
        request (HttpRequest): The HTTP request containing POST data for
                               the product update.

    Returns:
        JsonResponse: A JSON object with the status and message of the
        operation.
    """
    if request.method == "POST":
        ...
        prix_promotion = request.POST.get(
            "prixPromotion", prix
        )  # Utiliser le prix normal si le prix promotionnel n'est pas fourni
        ...
        if not all([id_produit,
                    ...
                    seuil_min,
                    categorie_id]):
            reponse["message"] = "Tous les champs doivent être remplis."
        ...
        try:
            ...
            if image:
                ...
                if not any(
                    image.name.lower().endswith(ext) for ext in allowed_extensions
                ):
                    reponse["message"] = (
                        "Veuillez choisir une image en JPG ou PNG."
                    )
                    return JsonResponse(reponse)

                # Enregistrer l'image dans le dossier approprié
                image_path = default_storage.save(
                    f"admins_epicerie/static/image/photos_produits/{image.name}",
                    image,
                )
        ...
        return JsonResponse(
            {"status": "Echec", "message": "Méthode non autorisée."},
            status=405,
        )
...
def ajout_categorie(request):
    ...
    Handles POST requests to create a new category based on the provided name.
    Returns a JSON response indicating success or failure.

    Args:
        request (HttpRequest): The HTTP request containing the category name
                               in the POST data.

    Returns:
        JsonResponse: A JSON response containing either an error message or
                      a success code (0 for success).
    """
...
def ajout_produit(request):
    ...
    Processes a POST request containing product details, including name,
    description, price, quantity, minimum threshold, category, image,
    promotion price, owner, and author. Saves the product to the database
    if all required fields are provided and valid.

    Args:
        request (HttpRequest): The HTTP request containing product details
                               in the POST data and files.

    Returns:
        JsonResponse: A JSON response with a success code (0 for success)
                      or an error message.
    """
    reponse = ""
    ...
    if request.method == "POST":
        ...
        if (
            nom and description and prix and qte and seuil_min and
            categorie_id and image
        ):
            try:
                ...
                return JsonResponse(
                    "Veuillez entrer des valeurs numériques valides pour le prix, "
                    "la quantité ou le seuil minimum.",
                    safe=False,
                )
            ...
            if file_extension in allowed_extensions:
                ...
                image_path = os.path.join(
                    "admins_epicerie/static/image/photos_produits/", image.name
                )
                with open(
                    os.path.join(settings.MEDIA_ROOT, image_path), "wb+"
                ) as destination:
...
def deconnexion_admin(request):
    """
    Handles the disconnection of the currently logged-in admin.

    If the admin is logged in, updates the admin's last connection date
    and status to 'DECONNECTE', clears the session data, and redirects
    to the homepage.
    ...
    Returns:
        HttpResponseRedirect: Redirects to the homepage (index).
    """
